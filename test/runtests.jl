...
@testset "Correctness (manual)" begin
    links = extract_links(read(joinpath(@__DIR__, "pages", "www.julialang.org"), String), "https://www.julialang.org")
    for str in [
    ...
    links = extract_links(read(joinpath(@__DIR__, "pages", "nano.org"), String), "https://nano.org")
...
@testset "Consistency" begin
    for file in readdir(joinpath(@__DIR__, "pages"))
        actual = extract_links(read(joinpath(@__DIR__, "pages", file), String), "https://"*replace(file, "%2F" => "/"))
...
@testset "Errors" begin
    @test_throws ArgumentError extract_links("body", "root")
end
